...
function BranchCard({image, title, address, withHoverEffect = true}) {
  ...
  return (
    <>
      <div className="relative flex sm:flex-col items-start sm:items-center sm:max-w-72 w-full h-36 sm:h-auto border-2 rounded-xl text-center group sm:pb-2 cursor-pointer hover:shadow-card">
        <div className="w-full h-full relative">
          <img
            src={image}
            alt={title}
            className="w-full h-full  sm:h-48  object-cover rounded-s-xl sm:rounded-b-none sm:rounded-t-xl"
          />
          {withHoverEffect && (
            <>
              <div className="absolute top-0 left-0 w-full h-full bg-black bg-opacity-50 rounded-t-xl opacity-0 group-hover:opacity-100 transition-opacity"></div>
              <button
                onClick={openGallery}
        ...
        <div
          className={cn(
            ' w-full p-2',
            withHoverEffect
              ? 'relative transition-all -bottom-6 sm:group-hover:bottom-0'
              : 'sm:bottom-0'
          )}
        >
